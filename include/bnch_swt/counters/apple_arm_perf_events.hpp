...
namespace bnch_swt::internal {
	...
	template<typename value_type> constexpr size_t lib_nelems(const value_type& x) {
		return sizeof(x) / sizeof((x)[0]);
	}

	#define lib_symbol_def(name) \
		lib_symbol { \
			#name, std::bit_cast<void**>(&name) \
		}
	...
	BNCH_SWT_HOST static bool lib_init() {
		...
		if (!lib_handle_kperf) {
			...
			std::string msg = "Failed to load kperf.framework: ";
			msg += (err ? err : "unknown");
			std::strncpy(lib_err_msg, msg.c_str(), sizeof(lib_err_msg) - 1);
		...
		if (!lib_handle_kperfdata) {
			const char* err = dlerror();
			std::string msg = "Failed to load kperfdata.framework: ";
			msg += (err ? err : "unknown");
			std::strncpy(lib_err_msg, msg.c_str(), sizeof(lib_err_msg) - 1);
			return failure_cleanup();
		}

		if (!load_symbols(lib_handle_kperf, lib_symbols_kperf, "kperf")) {
			return failure_cleanup();
		}

		if (!load_symbols(lib_handle_kperfdata, lib_symbols_kperfdata, "kperfdata")) {
			return failure_cleanup();
		}

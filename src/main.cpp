...
static constexpr uint64_t max_iteration_count{ 10000 };
...
static std::vector<uint8_t> make_corpus_impl(const std::vector<utf8_entry>& table, size_t n) {
...
static std::vector<std::vector<uint8_t>> make_corpus(const std::vector<utf8_entry>& table, size_t n) {
	std::vector<std::vector<uint8_t>> return_values{};
	return_values.resize(100);
	for (uint64_t x = 0; x < 100; ++x) {
		return_values[x] = make_corpus_impl(table, n);
	}
...
static int run_correctness_tests() {
	...
	auto test = [&](const char* label, std::vector<uint8_t> buf, bool expected) {
		bool jsonifier_result	= jsonifier::validateUtf8(buf.data(), buf.size());
		bool simdjson_result = simdjson::validate_utf8(reinterpret_cast<const char*>(buf.data()), buf.size());
		bool ok		= (jsonifier_result == expected) && (jsonifier_result == simdjson_result);
		if (!ok) {
			++failures;
			std::cout << "FAIL [" << label << "]: jsonifier_result=" << jsonifier_result << " simdjson=" << simdjson_result << " expected=" << expected << std::endl;
		}
	};
	test("empty", {}, true);
...
struct simdjson_validate {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<std::vector<uint8_t>>& buf_new, uint64_t& index) {
		auto& buf = buf_new[index % buf_new.size()];
		if (auto result = simdjson::validate_utf8(reinterpret_cast<const char*>(buf.data()), buf.size()); result) {
...
struct jsonifier_validate_multibyte {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<std::vector<uint8_t>>& buf_new, uint64_t& index) {
		auto& buf = buf_new[index % buf_new.size()];
		if (!jsonifier::validateUtf8Multibyte(buf.data(), buf.size())) {
...
struct jsonifier_validate_ascii {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<std::vector<uint8_t>>& buf_new, uint64_t& index) {
		auto& buf = buf_new[index % buf_new.size()];
		if (!jsonifier::validateUtf8Ascii(buf.data(), buf.size())) {
			throw std::runtime_error{ "Jsonifier Failed validation!" };
		}
		return buf.size();
	}
...
struct jsonifier_validate_mixed {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<std::vector<uint8_t>>& buf_new, uint64_t& index) {
		auto& buf = buf_new[index % buf_new.size()];
		if (!jsonifier::validateUtf8Mixed(buf.data(), buf.size())) {
			throw std::runtime_error{ "Jsonifier Failed validation!" };
		}
		return buf.size();
	}
...
int main() {
	try {
		...
		static constexpr stage_config_data config{ [] {
			...
			return_data.max_iteration_count				   = max_iteration_count;
			return_data.clear_cpu_caches_before_iterations = true;
			return_data.measured_iteration_count		   = 20;
			return return_data;
		}() };
		...
		stage_ascii ::run_benchmark<"utf8-validate", "simdjson", simdjson_validate>(ascii_buf, current_index);
		current_index = 0;
		stage_ascii ::run_benchmark<"utf8-validate", "jsonifier", jsonifier_validate_ascii>(ascii_buf, current_index);
		current_index = 0;
		stage_mixed ::run_benchmark<"utf8-validate", "simdjson", simdjson_validate>(mixed_buf, current_index);
		current_index = 0;
		stage_mixed ::run_benchmark<"utf8-validate", "jsonifier", jsonifier_validate_mixed>(mixed_buf, current_index);
		current_index = 0;
		stage_multibyte::run_benchmark<"utf8-validate", "simdjson", simdjson_validate>(multibyte_buf, current_index);
		current_index = 0;
		stage_multibyte::run_benchmark<"utf8-validate", "jsonifier", jsonifier_validate_multibyte>(multibyte_buf, current_index);

...
static int run_correctness_tests() {
	...
	auto test = [&](const char* label, std::vector<uint8_t> buf, bool expected) {
		bool ours	= jsonifier_internal::validate_utf8_mixed(buf.data(), buf.size());
		bool theirs = simdjson::validate_utf8(reinterpret_cast<const char*>(buf.data()), buf.size());
		bool ok		= (ours == expected) && (ours == theirs);
		if (!ok) {
			++failures;
			std::cout << "FAIL [" << label << "]: ours=" << ours << " simdjson=" << theirs << " expected=" << expected << std::endl;
		}
	};
	test("empty", {}, true);
...
struct jsonifier_validate_multibyte {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<uint8_t>& buf) {
		if (!jsonifier_internal::validate_utf8_multibyte(buf.data(), buf.size()))
...
struct jsonifier_validate_ascii {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<uint8_t>& buf) {
		if (!jsonifier_internal::validate_utf8_ascii(buf.data(), buf.size()))
			throw std::runtime_error{ "Jsonifier Failed validation!" };
		return buf.size();
	}
};
...
struct jsonifier_validate_mixed {
	BNCH_SWT_HOST static uint64_t impl(const std::vector<uint8_t>& buf) {
		if (!jsonifier_internal::validate_utf8_mixed(buf.data(), buf.size()))
			throw std::runtime_error{ "Jsonifier Failed validation!" };
		return buf.size();
	}
};
